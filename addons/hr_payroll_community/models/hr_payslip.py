...
class HrPayslip(models.Model):
    ...
    is_monthly = fields.Boolean(string='Monthly', compute='_set_compute_monthly', store=True, readonly=False)

    @api.depends('payslip_run_id')
    def _set_compute_monthly(self):
        for dat in self:
            if dat.payslip_run_id:
                dat.is_monthly = dat.payslip_run_id.is_monthly
    ...
    @api.model
    def get_worked_day_lines(self, contracts, date_from, date_to, is_monthly=False):
        """
        ...
        for contract in contracts.filtered(lambda contract: contract.resource_calendar_id):
            ...
            for day, hours, leave in day_leave_intervals:
                ...
                # for dat in leave:
                #     if not dat.holiday_id:
                #         total_global += 1
                # holiday.number_of_days = holiday.number_of_days if total_global < 1 else total_global
                if today != day:
                    today = day
                elif today == day:
                    continue
                ...
                current_leave_struct['name'] += ', ' + day.strftime('%d')
                ...
                if work_hours:
                    ...
                    # current_leave_struct['number_of_days']  = holiday.number_of_days
                    # if holiday.number_of_days < 1:
                    #     current_leave_struct['number_of_days'] = holiday.number_of_days
                    # else:
                    current_leave_struct['number_of_days'] += 1
    ...
    def get_inputs(self, contracts, date_from, date_to, is_monthly=False):
        ...
        # if self.is_monthly:
        #     sorted_rule_ids = filter(lambda x: x.is_monthly, sorted_rule_ids)
    ...
    def _get_payslip_lines(self, contract_ids, payslip_id):
        ...
        year_mod = date.today().year % 2000
        start_period = date(year, month, 1)
        end_period = self.last_day_of_month(date.today())
        time_off = self.env['hr.leave'].search(
            [('employee_id', '=', payslip.employee_id.id), ('date_from', '>=', start_period),
             ('date_to', '<=', end_period), ('state', '=', 'validate'), ('payslip_status', '=', True)])

        # TODO: SUBTRACT MEAL & DILIGENCE ALLOWANCE
        count_forgot_absent = 0
        count_late_unpaid = 0
        count_half_day = 0
        count_late = 0
        count_sick = 0
        ...
        for off in time_off:
            if off.holiday_status_id.code == 'telat'+str(year):
            #     count_late_unpaid += off.number_of_days
                count_late += off.number_of_days
            elif off.holiday_status_id.code == 'izin_ptg_um_'+str(year):
                count_half_day += off.number_of_days
            elif off.holiday_status_id.code == 'cuti_tdk_byr_'+str(year_mod):
                count_off += off.number_of_days
            elif off.holiday_status_id.code == 'skt_pot_kerajinan':
                count_sick += 1
            elif off.holiday_status_id.code == 'cuti'+str(year) and off.subtract_diligence:
                count_off += off.number_of_days
        ...
        if payslip.is_monthly:
            sorted_rules = filter(lambda x: x.is_monthly, sorted_rules)
        else:
            sorted_rules = filter(lambda x: not x.is_monthly, sorted_rules)
            ...
            for rule in sorted_rules:
                ...
                if rule._satisfy_condition(localdict) and rule.id not in blacklist:
                    ...
                    if rule.code == 'DILIGENCE':
                        if datetime.now().year - contract.date_start.year >= 6 and tot_rule != 0:
                            tot_rule += 100000
                        if (count_half_day > 2 or count_late > 3 or count_off > 0 or count_sick > 0):
                            tot_rule = 0
                    # if rule.code == 'WORK100' and (count_forgot_absent > 0 or count_late > 0):
                    #     tot_rule /= 2
    ...
    def onchange_employee_id(self, date_from, date_to, employee_id=False, contract_id=False, is_monthly=False):
        # defaults
        res = {
            'value': {
                'line_ids': [],
        ...
        contracts = self.env['hr.contract'].browse(contract_ids)
        worked_days_line_ids = self.get_worked_day_lines(contracts, date_from, date_to, is_monthly)
        input_line_ids = self.get_inputs(contracts, date_from, date_to, is_monthly)
        res['value'].update({
    ...
    def onchange_employee(self):
        ...
        # computation of the salary input
        contracts = self.env['hr.contract'].browse(contract_ids)
        worked_days_line_ids = self.get_worked_day_lines(contracts, date_from, date_to, self.is_monthly)
        worked_days_lines = self.worked_days_line_ids.browse([])
        for r in worked_days_line_ids:
            worked_days_lines += worked_days_lines.new(r)
        self.worked_days_line_ids = worked_days_lines

        input_line_ids = self.get_inputs(contracts, date_from, date_to, self.is_monthly)
...
class HrPayslipInput(models.Model):
    ...
    contract_id = fields.Many2one('hr.contract', string='Contract', required=True,
                                  help="The contract for which applied this input")
    number_of_days = fields.Integer(string='Jumlah Hari')
    number_of_hours = fields.Integer(string='Durasi Lembur')
...
class HrPayslipRun(models.Model):
    ...
    credit_note = fields.Boolean(string='Credit Note', readonly=True,
                                 ...
                                 help="If its checked, indicates that all payslips generated from here are refund "
                                      "payslips.")
    is_monthly = fields.Boolean(string='Monthly')
...
class ResourceMixin(models.AbstractModel):
    ...
    def get_work_days_data(self, from_datetime, to_datetime, compute_leaves=False, calendar=None, domain=None):
        """
