...
class PrintCashSummary(models.TransientModel):
    ...
    cash_account_ids = fields.Many2many('account.account',
    ...
    def action_print_cash_summary(self,):
        ...
        for wizard in self:
            ...
            #c_account_id =[]      
            c_account_id = wizard.cash_account_ids.ids
            journal_ids = self.env['account.journal'].search([('default_debit_account_id','in',c_account_id)])
            #i_acc_name = journal_ids.default_debit_account_id.ids
            #for journal in journal_ids:
            #    i_acc_name += journal.default_debit_account_id.name

            c_domain += [('journal_id','in',journal_ids.ids)]
            i_domain += [('account_id', 'in', c_account_id)] #journal_ids.default_debit_account_id.ids)]
            ...
            for res in summary_objs:    
                is_cash = False
                acc_name = res["account_id"][1]
                acc_name = acc_name._value
                for acc_id in c_account_id :
                ...
                if res['account_id'][0] == 1 or res['account_id'][0] == 923:
            ...
            for invoice in invoice_objs.sorted(key=lambda m: m.date):
                is_cash = False
                #acc_name = res["account_id"][1]
                #acc_name = invoice.acc_id.display_name
                #if invoice.account_id in c_account_id:
                #    raise UserError(_('Customs Data is not set yet'))
                for acc_id in c_account_id :
                    #raise Warning(_('journal : ' + acc_name + ' '+ str(invoice.account_id) + ' '+str(acc_id)))
                    #raise UserError(_('Customs Data is not set yet'))
                    if acc_id == invoice.account_id.id:                        
                        is_cash = True
                ...
                statement_date = ''
                if invoice.statement_id.date:
                    statement_date = invoice.statement_id.date.strftime('%d-%m-%Y')
                #else:
                #   statement_date = ''
                
                initial_balance += invoice.credit
                initial_balance -= invoice.debit
