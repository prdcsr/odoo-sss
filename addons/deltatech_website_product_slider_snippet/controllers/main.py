...
class WebsiteSale(Base):
    ...
    def _get_products_slider(self, list_id):
        ...
        Product = request.env["product.product"].with_context(display_default_code=True)
        ...
        FieldMonetary = request.env["ir.qweb.field.float"]
        monetary_options = {
            "precision" :0,
            "display_currency": request.website.get_current_pricelist().currency_id,
        }
        ...
        for product in products_ids:
            ...
            res_product = product.read(["id", "name", "website_url","default_code","web_pro_name","public_categ_ids"])[0]
            res_product.update(combination_info)
            res_product["price"] = FieldMonetary.value_to_html(res_product["price"], monetary_options)
            #web_info = request.env["website"]
            #for website in web_info:
            #    res_product["logo"] = website.logo
            res_product["categ_name"] = product.public_categ_ids.name
            if rating:
