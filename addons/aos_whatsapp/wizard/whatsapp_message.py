...
#             
    ...
    def whatsapp_message_post_new(self, KlikApi):
        try:
            ...
            #print ('===mass_whatsapp_opt_out_list==',context)
                        ...
                        for record in self.env[active_model].browse(active_ids):
                            ...
                            if partner.whatsapp and partner.whatsapp != '0' and whatsapp not in opt_out_list:
                                ...
                                message_data = {
                                    ...
                                    'body': message.replace('_PARTNER_', partner.name).replace('_NUMBER_', origin).replace('_AMOUNT_TOTAL_', str(self.format_amount(amount_total, currency_id)) if currency_id else '').replace('\xa0', ' '),
                                    'origin': origin,
                                    'link': link,
                                }
                                #print ('===',message_data)
                                ...
                                if attachment_new_ids:
                                    for att in attachment_new_ids:
                                        ...
                                        if message_attach['body']:
                                            ...
                                            vals = self._prepare_mail_message(self.env.user.partner_id.id, chatID, record and record.id, active_model, '', message_attach, record.name, [partner.id], attachments, send_attach, status)
                                            MailMessage.sudo().create(vals)
                                            #partner.chat_id = chatID
                                            #new_cr.commit()
                                #print ('=END=',rec)
