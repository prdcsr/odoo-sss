	...
	init: function(parent, context) {
        ...
        if (context.tag == 'sale_dashboard') {
            self._rpc({
                ...
                /*var this_year_sales_amount = (self.sale_data[5]['amount_total_this_month']).toFixed(2)
                self.sale_data.push(this_year_sales_amount); 
                var currency  = self.sale_data[6]
                this_year_sales_amount = self.format_currency(currency, this_year_sales_amount);
                $('#this_year_sales_amount').attr("value",this_year_sales_amount);
                $('#this_year_sales_amount').attr("max", this_year_sales_amount);
                $('#this_year_sales_amount').append('<div class="logo">' + '<span>' + this_year_sales_amount + '</span></div>');*/
            }).then(function(){
                self.render();
...
// format sales data    
    /*f_saledata : function () {
        var self = this
        var s_month = this.$el.find('#sale_this_month')
        var s_monthrp = new Intl.NumberFormat("id-ID", {
            style: "currency",
            currency: "IDR"
        }).format(self.sale_data[5]['amount_total_this_month'])
        s_month.append('<span>s_monthrp</span>')
    }*/
//     Here we are plotting bar,pie chart
  graph: function() {
        var self = this
        
        // Format n Display Data Sales
        var current_sale_amount = this.$el.find('#current_sale_amount')
        var s_day = this.$el.find('#s_day_')
        var s_dayrp = new Intl.NumberFormat("id-ID", {
            style: "currency",
            currency: "IDR"
        }).format(self.sale_data[7]['total_amount_invoiced'])
        s_day.append('<span>'+s_dayrp+'</span>');
        current_sale_amount.append('<li><div>Today</div><div>'+s_dayrp+'</div></li>');

        var s_month = this.$el.find('#s_month_')
        var s_monthrp = new Intl.NumberFormat("id-ID", {
            style: "currency",
            currency: "IDR"
        }).format(self.sale_data[5]['amount_total_this_month'])
        s_month.append('<span>'+s_monthrp+'</span>');
        current_sale_amount.append('<li><div>This Month</div><div>'+s_monthrp+'</div></li>');
        
        var s_year = this.$el.find('#s_year_')
        var s_yearrp = new Intl.NumberFormat("id-ID", {
            style: "currency",
            currency: "IDR"
        }).format(self.sale_data[4]['total_amount_this_year'])
        s_year.append('<li><div>This Year</div><div>'+s_yearrp+'</div></li>')
        current_sale_amount.append('<li><div>This Year</div><div>'+s_yearrp+'</div></li>');;
