...
class Partner(models.Model):
    ...
    whatsapp = fields.Char('Whatsapp (Input with country code)', default='0', help="Please input number with country code", )
    whatsapp_type = fields.Selection([('@c.us','Contact'),('@g.us','Group')], string='Whatsapp Type', default='@c.us'
                                     , required=True)
    chat_id = fields.Char(string='ChatID')
    sale_order_count = fields.Char(string='Sale Order Count')

    @api.constrains('whatsapp')
    def _check_number(self):
        for rec in self:
            if rec.whatsapp:
                is_digits_only = re.match(r'^[0-9]+$', rec.whatsapp)
                if not is_digits_only:
                    raise ValidationError(_('Whatsapp number must only contains numbers.'))
                if len(rec.whatsapp) < 11:
                    raise ValidationError(_("Whatsapp number minimal 11 digits number "))

    def send_whatsapp(self):
        return {'type': 'ir.actions.act_window',
                'name': _('Whatsapp Message'),
                'res_model': 'green.whatsapp.compose.message',
                'target': 'new',
                'view_mode': 'form',
                'view_type': 'form',
    ...
    def check_whatsapp_number_response(self, whatsapp_ids):
        ...
        if self.whatsapp:
            ...
            GreenApi = whatsapp_ids.greenapi()
            number_dict = GreenApi.get_phone(method='checkPhone', phone=whatsapp)
        return number_dict

    def check_number_whatsapp(self):
        """Check Partner Mobile."""
        WhatsappServer = self.env['ir.green_whatsapp_server']
        whatsapp_ids = WhatsappServer.search([('status','=','authorized')])
        for rec in self:
