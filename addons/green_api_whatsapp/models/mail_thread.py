...
class MailThread(models.AbstractModel):
    ...
    def _message_whatsapp(self, body, subtype_id=False, partner_ids=False, number_field=False,
        ...
        for partner in partners:
            ...
            if partner.whatsapp and partner.whatsapp != '0':
                message_data = {
                    ...
                    'body': plaintext2html(html2plaintext(body.replace('_PARTNER_', partner.name).replace('"', "*").replace("'", "*"))),#.replace('_PARTNER_', partner.name).replace('_NUMBER_', origin).replace('_AMOUNT_TOTAL_', str(self.format_amount(amount_total, currency_id)) if currency_id else '').replace('\xa0', ' '),
        ...
        # if subtype_id is False:
        #     subtype_id = self.env['ir.model.data'].xmlid_to_res_id('mail.mt_note')
        # #{'method': 'sendMessage', 'phone': '626285863963913', 'chatId': '', 'body': 'test', 'origin': '', 'link': ''}
