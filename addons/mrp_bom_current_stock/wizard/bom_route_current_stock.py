...
class BomRouteCurrentStock(models.TransientModel):
    ...
    plan_qty =  fields.Float(
        String ="Production Plan", digits="Product Unit of Measure",Default = 1,
    )
    ...
    line_ids = fields.One2many(
        comodel_name="mrp.bom.current.stock.line", inverse_name="explosion_id",domain="[('product_id.default_code[:5]', '!=', 'PRCOM')]"
    )
...
class BomRouteCurrentStockLine(models.TransientModel):
    ...
    def _compute_qty_available_in_source_loc(self):
        for record in self:
            ...
            record.qty_req = record.product_qty * record.explosion_id.plan_qty
            record.qty_diff = record.qty_available_in_source_loc - record.qty_req
