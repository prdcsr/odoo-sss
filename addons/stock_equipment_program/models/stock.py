...
class Picking(models.Model):
    ...
    applied_coupon_ids = fields.One2many('sale.coupon', 'stock_picking_id', string="Applied Coupons", copy=False)
    generated_coupon_ids = fields.One2many('sale.coupon', 'picking_id', string="Offered Coupons", copy=False)
    ...
    def _get_reward_lines(self):
        self.ensure_one()
        return self.move_ids_without_package.filtered(lambda line: line.is_reward_line)

    def _is_reward_in_order_lines(self, program):
        self.ensure_one()
        order_quantity = sum(self.move_ids_without_package.filtered(lambda line:
                                                                    line.product_id == program.reward_product_id).mapped(
            'product_uom_qty'))
    ...
    def _get_reward_values_product(self, program):
        # price_unit = self.move_ids_without_package.filtered(lambda line: program.reward_product_id == line.product_id)[0].price_reduce

        order_lines = (self.move_ids_without_package - self._get_reward_lines()).filtered(
            lambda x: program._get_valid_products(x.product_id))
        max_product_qty = sum(order_lines.mapped('product_uom_qty')) or 1
        total_qty = program.reward_product_quantity * max_product_qty
        # total_qty = sum(
        #     self.move_ids_without_package.filtered(lambda x: x.product_id == program.reward_product_id).mapped('product_uom_qty'))
        ...
        if purchase_order:
            stock_move.update({
                ...
                'partner_id': purchase_order.dest_address_id.id,
                'route_ids': purchase_order.picking_type_id.warehouse_id and [
                    (6, 0, [x.id for x in purchase_order.picking_type_id.warehouse_id.route_ids])] or [],
                ...
                'move_dest_ids': [(4, x) for x in self.move_ids_without_package[0].move_dest_ids.ids] if self.move_ids_without_package else [],
            })
    ...
    def _get_paid_order_lines(self):
        ...
        free_reward_product = self.env['sale.coupon.program'].search([('reward_type', '=', 'product')]).mapped(
    ...
    def _get_base_order_lines(self, program):
        ...
        """
        return self.move_ids_without_package.filtered(
            lambda x: not (x.is_reward_line and x.product_id == program.discount_line_product_id))
    ...
    def _get_cheapest_line(self):
        ...
        return min(self.move_ids_without_package.filtered(lambda x: not x.is_reward_line and x.price_reduce > 0),
    ...
    def _get_max_reward_values_per_tax(self, program, taxes):
        lines = self.move_ids_without_package.filtered(
            lambda l: l.tax_id == taxes and l.product_id != program.discount_line_product_id)
    ...
    def _get_reward_values_percentage_amount(self, program):
        ...
        self.move_ids_without_package.filtered(lambda l: l.product_id in fixed_price_products).write({'price_unit': 0})
    ...
    def _get_applicable_programs(self):
        ...
        ], order="id")._filter_picking_programs_from_common_rules(self)
        # no impact code...
    ...
    def _get_applicable_no_code_promo_program(self):
        ...
        ])._filter_picking_programs_from_common_rules(self)
        return programs
    ...
    def _get_valid_applied_coupon_program(self):
        ...
        programs = self.applied_coupon_ids.mapped('program_id').filtered(
            lambda p: p.promo_applicability == 'on_next_order')._filter_picking_programs_from_common_rules(self, True)
        programs += self.applied_coupon_ids.mapped('program_id').filtered(
            lambda p: p.promo_applicability == 'on_current_order')._filter_picking_programs_from_common_rules(self)
        return programs
    ...
    def _create_new_no_code_promo_reward_lines(self):
        ...
        for program in programs:
            ...
            elif program.discount_line_product_id.id not in self.move_ids_without_package.mapped('product_id').ids:
                self.write({'move_ids_without_package': [(0, False, value) for value in
    ...
    def _update_existing_reward_lines(self):
        ...
        for program in applied_programs.sorted(
                lambda ap: (ap.discount_type == 'fixed_amount', ap.discount_apply_on == 'on_order')):
            values = order._get_reward_line_values(program)
            lines = order.move_ids_without_package.filtered(
    ...
    def _remove_invalid_reward_lines(self):
        ...
        invalid_lines = order.move_ids_without_package.filtered(
        ...
        if programs_to_remove:
            ...
            if product_ids_to_remove:
                ...
                self.generated_coupon_ids.filtered(
                    lambda coupon: coupon.program_id.discount_line_product_id.id in product_ids_to_remove).write(
                    {'state': 'expired'})
...
class SaleOrderLine(models.Model):
    ...
    def unlink(self):
        ...
        for line in self.filtered(lambda line: line.is_reward_line):
            ...
            if related_program:
                line.picking_id.no_code_promo_program_ids -= related_program
                line.picking_id.code_promo_program_id -= related_program
                related_program_lines |= line.picking_id.move_ids_without_package.filtered(
    ...
    # Another possibility is to add on product.product a one2many to stock.move 'order_line_ids',
