...
class PurchaseOrder(models.Model):
    ...
    requesting_operating_unit_id = fields.Many2one(
        ...
        # compute='_compute_requesting_operating_unit',
        # store=True
    )

    discount_total = fields.Float(string="Discount Total", compute="_compute_discount_dp_total")
    dp_total = fields.Float(string="DP Total", compute="_compute_discount_dp_total")

    @api.depends("order_line")
    def _compute_discount_dp_total(self):
        self.ensure_one()
        total_discount = 0.0
        total_dp = 0.0
        for line in self.order_line:
            if line.product_id.default_code == 'PDisc':
                total_discount += line.price_subtotal
            if line.product_id.default_code == 'DP - Purchase':
                total_dp += line.price_unit
        self.discount_total = total_discount
        self.dp_total = total_dp

    @api.depends("user_id")
    def _compute_operating_unit(self):
        for dat in self:
            if dat.user_id:
                dat.operating_unit_id = dat.user_id.default_operating_unit_id

    # @api.depends("user_id")
    # def _compute_requesting_operating_unit(self):
    #     for dat in self:
    #         if dat.user_id:
    #             dat.requesting_operating_unit_id = dat.user_id.default_operating_unit_id
