...
class PaymentRequest(models.Model):
    ...
    def _onchange_amount_dpp(self):      
        ...
        for rec in self:
            vat = int(math.ceil(((rec.amount_dpp + rec.amount_duty)  * 0.1)/1000)) * 1000 
    ...
    @api.onchange("amount_tax_vat","amount_tax_income")
    def _onchange_amount_dpp(self):      
   ...
   # @api.multi
    def action_confirm(self):
        ...
        channel_all_employees = self.env.ref('AG_payment_request.channel_all_payment_request').read()[0]
        template_new_employee = self.env.ref('AG_payment_request.email_template_data_payment_request').read()[0]
   ...
   # @api.multi
    def action_department_approve(self):
        ...
        channel_all_employees = self.env.ref('AG_payment_request.channel_all_to_approve_payment_request').read()[0]
        template_new_employee = self.env.ref('AG_payment_request.email_template_data_to_approve_payment_request').read()[0]
        # raise ValidationError(_(template_new_employee))
        if template_new_employee:
            # MailTemplate = self.env['mail.template']
            body_html = template_new_employee['body_html']
