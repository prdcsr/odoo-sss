...
class FnbPromoSchedule(models.Model):
    ...
    time_start = fields.Datetime(string="Time From")
    time_end = fields.Datetime(string="Time To")
    promo_lines = fields.One2many('fnb.promo.line', 'promo_id')
...
class FnbPromoLine(models.Model):
    _name = "fnb.promo.line"

    name = fields.Char("Description", required=True)
    product_id = fields.Many2one("product.product", "Product", required=True)
    product_tmplt_id = fields.Many2one("product.template")
    max_qty = fields.Float("Max Quantity Usage")
    # qty = fields.Float("Quantity")
    uom_id = fields.Many2one('uom.uom', "UoM", required=True)
    promo_id = fields.Many2one('fnb.promo.schedule', "Promo", required=True)
    location_id = fields.Many2one('stock.location', domain="[('usage', '=', 'internal')]", required=True)

    @api.onchange("product_id")
    def _onchange_product_id(self):
        if self.product_id:
            self.product_tmplt_id = self.product_id.product_tmpl_id
    ...
    def _onchange_uom_id(self):
        for line in self:
            if line.product_id and line.uom_id:
                if line.uom_id.measure_type != line.product_id.uom_id.measure_type:
