...
class AccountMove(models.Model):
    ...
    def _constraint_kode_ppn(self):
        ...
        for move in self.filtered(lambda m: m.l10n_id_kode_transaksi != '08'):
            if any(ppn_tag.id in line.tag_ids.ids for line in move.line_ids if line.exclude_from_invoice_tab is False and line.display_type is False) and any(ppn_tag.id not in line.tag_ids.ids for line in move.line_ids if line.exclude_from_invoice_tab is False and line.display_type is False):
    ...
    def post(self):
        ...
        for move in self:
            if move.l10n_id_need_kode_transaksi and move.operating_unit_id.name != 'JAMBI':
                ...
                if move.l10n_id_replace_invoice_id.l10n_id_tax_number:
                    ...
                    if move.operating_unit_id:
                        efaktur = self.env['l10n_id_efaktur.efaktur.range'].pop_number(move.company_id.id,move.operating_unit_id.id)
                    else:
    ...
    def _generate_efaktur_invoice(self, delimiter):
        ...
        for move in self.filtered(lambda m: m.state == 'posted'):
            ...
            #eTax['NAMA'] = move.partner_id.l10n_id_nik + "#NIK#NAMA#" + move.partner_id.name if eTax['NPWP'] == '000000000000000' else move.partner_id.l10n_id_tax_name or move.partner_id.name
            eTax['NAMA'] = move.partner_id.l10n_id_tax_name or move.partner_id.name
            ...
            eTax['ALAMAT_LENGKAP'] = move.partner_id.l10n_id_tax_address or street
            ...
            lines = move.line_ids.filtered(lambda x: x.product_id.id == int(dp_product_id) and x.price_subtotal < 0)
            if lines:
                eTax['FG_UANG_MUKA'] = 2
                eTax['UANG_MUKA_DPP'] = eTax['JUMLAH_DPP']
                eTax['UANG_MUKA_PPN'] = eTax['JUMLAH_PPN']
            ...
            for line in move.line_ids.filtered(lambda l: not l.exclude_from_invoice_tab):
                ...
                line_dict = {
                    ...
                    # 'NAMA': line.name or '',
                    'NAMA': line.product_id.display_name or '',
                    'HARGA_SATUAN': int(invoice_line_unit_price),
                    'JUMLAH_BARANG': line.quantity,
                    'HARGA_TOTAL': int(invoice_line_total_price),
                    'DPP': int(line.price_subtotal) if move.l10n_id_kode_transaksi != '09' else int(11/12 * line.price_subtotal),
            ...
            len_sales = len(sales)
            b_diskon = l_diskon
            maxdiskpct = round((l_diskon/eTax['JUMLAH_DPP'])+0.005,2)
            for sale in sales:
                #f_diskon = int(b_diskon/len_sales)
                f_diskon = b_diskon
                #bila diskon lebih besar dari 15% DPP
                if (f_diskon + sale['DISKON']) > int(sale['DPP']*maxdiskpct):
                    f_diskon = int(sale['DPP']*maxdiskpct)
                f_diskon -= sale['DISKON']
                b_diskon -= f_diskon
                len_sales -= 1
                sale['DISKON'] += f_diskon
                sale['DPP'] -= f_diskon
                tax_line = 0

                temp_dpp = 0
                if move.l10n_id_kode_transaksi == '04':
                    temp_dpp = sale['DPP'] * 11 / 12
                    sale['DPP'] = round(sale['DPP'] * 11/12)

                for tax in line.tax_ids:
                    if tax.amount > 0:
                        tax_line += round(temp_dpp * (tax.amount / 100.0))
                sale['PPN'] = int(tax_line)

                """ for f in free:
                    #if f['product_id'] == sale['product_id']:
                    f_diskon=int(f['DISKON']/len(sales))
                    f_diskon += b_diskon
                    if abs(f_diskon) > sale['DPP']:
                        b_diskon = sale['DPP'] + f_diskon #f['DPP']
                        f_diskon = sale['DPP'] * -1
                    else:
                        b_diskon = 0
                    #sale['DISKON'] = sale['DISKON'] -  f['diskon']+ f['PPN']
                    sale['DISKON'] = sale['DISKON'] -  f_diskon
                    sale['DPP'] = sale['DPP'] + f_diskon #f['DPP']
            ...
            ppn_diff = eTax['JUMLAH_PPN'] - sub_total_ppn_before_adjustment
            len_sales = len(sales)-1
            if ppn_diff != 0:
                #lines = move.line_ids.filtered(lambda x: x.product_id.id == int(dp_product_id) and x.price_subtotal < 0)
                if lines:
                    eTax['JUMLAH_DPP'] = int(sub_total_before_adjustment)
                    eTax['JUMLAH_PPN'] = int(sub_total_ppn_before_adjustment)
                    #eTax['UANG_MUKA_DPP'] = eTax['JUMLAH_DPP'] - eTax['UANG_MUKA_DPP']
                    #eTax['UANG_MUKA_PPN'] = eTax['JUMLAH_PPN'] - eTax['UANG_MUKA_PPN']
                else:
                    sales[len_sales]['PPN'] += int(ppn_diff)
    ...
    def _prepare_etax(self):
        ...
        return {'JUMLAH_PPNBM': 0, 'UANG_MUKA_PPNBM': 0, 'BLOK': '', 'NOMOR': '', 'RT': '', 'RW': '', 'KECAMATAN': '', 'KELURAHAN': '', 'KABUPATEN': '', 'PROPINSI': '', 'KODE_POS': '', 'JUMLAH_BARANG': 0, 'TARIF_PPNBM': 0, 'PPNBM': 0,'KODE_DOKUMEN_PENDUKUNG' : 0}
