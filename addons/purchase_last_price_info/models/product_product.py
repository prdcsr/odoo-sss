...
class ProductProduct(models.Model):
    ...
    # Add januari 2024 For Sisa SC Report by Hidayat    
    ...
    last_purchase_qty = fields.Float(        
        compute="_compute_last_purchase_line_id_info",
        string="Last Purchase Qty",
    )

    qty_20ft = fields.Float(        
        #compute="_compute_last_purchase_line_id_info",
        string="Qty 20ft",
    )
    ...
    def _compute_last_purchase_line_id_info(self):
        for item in self:
            ...
            item.last_purchase_order_id = item.last_purchase_line_id.order_id
            item.last_purchase_qty = item.last_purchase_line_id.qty_received
    ...
    def _compute_last_purchase_price_currency(self):
        for item in self:
            if item.show_last_purchase_price_currency:
                ...
                #item.last_purchase_price_currency = item.last_purchase_currency_id.with_context(date= item.last_purchase_date).compute(amount, SAR)
