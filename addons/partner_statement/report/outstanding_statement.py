...
class OutstandingStatement(models.AbstractModel):
    ...
    def _display_lines_sql_q1(self, partners, date_end, account_type):
        ...
        return str(
            self._cr.mogrify(
                """
        SELECT l.id, m.name AS move_id, l.partner_id, l.date, l.name, ac.name as analytic,
        ...
        LEFT JOIN res_currency cur ON cur.id = l.currency_id
        LEFT JOIN account_analytic_account ac on ac.id = l.analytic_account_id
        ...
        GROUP BY l.id, l.partner_id, m.name, l.date, l.date_maturity, l.name,ac.name,
